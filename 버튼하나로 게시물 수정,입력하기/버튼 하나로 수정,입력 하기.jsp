...
<tr>
   ...
   <td style='border:1px solid green' valign=top>
      <table>
      <tr height=40px>
        <td><input type=hidden name ="hidden"id=hidden size=30></td>
         <td><input type=hidden name ="optype"id=optype size=30></td>
...
$(document).ready(function() {
	...
	$.get('selectAll',{},function(data){
		...
		$.each(data,function(ndx,value){
			...
			name='<option>'+value['staff_id']+','+value['NICK']+','+value['MOBILE']+','+value['GENDER']+'</option>';
...
.on('click','#btnInsert',function(){
	...
	$.ajax({
	 ...
	 if($("#optype").val()=="update"){
	   $.ajax({
		  url:'update',//호출될 서블릿의 경로(이름)
		  method:'get',//데이터 전송방식 get/post
		  data:'txtnick='+$('input[name=txtnick]').val()+'&txtmobile='+$('input[name=txtmobile]').val()+'&hidden='+$('input[name=hidden]').val()+'&gender='+$('input[name="gender"]:checked').val(),
		  dataType:'text',//반환받을 데이터 타입 
		  beforeSend:function(){},
		  success:function(txt){//호출 성공해서 완료하면 
			alert("입력완료되었습니다.");
